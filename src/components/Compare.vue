<template>
  <div
    class="flex flex-col items-center justify-start h-screen pt-5 space-y-6 bg-gray-100"
  >
    <div class="relative p-6 bg-white rounded-lg shadow-lg">
      ...
      <div class="relative w-full h-64">
        ...
        <video
          ...
          autoplay
          playsinline
          class="absolute top-0 left-0 w-full h-full bg-black rounded-lg"
      ...
      <div class="mb-4">
        ...
        <input
          v-model="number_person"
          type="text"
          class="w-full p-2 mt-1 border border-gray-300 rounded"
          placeholder="กรอกรหัสผู้ดูแล"
        />
        <div v-if="errors.number_person" class="text-red-500">
          {{ errors.number_person }}
        </div>
      </div>
      <div class="mb-4">
        ...
        <div v-if="errors.selectedRoom" class="text-red-500">
          {{ errors.selectedRoom }}
        </div>
      </div>
      <button
      ...
      <button
        @click="stopCamera"
        class="px-4 py-2 mx-3 mt-4 text-white bg-red-500 rounded hover:bg-red-600"
      >
        ปิดกล้อง
      </button>
      ...
      <div v-if="errors.faceEmbeddings" class="mt-4 text-red-500">
        {{ errors.faceEmbeddings }}
      </div>
      <button
        @click="submitForm"
        class="px-4 py-2 mt-4 text-white bg-purple-500 rounded hover:bg-purple-600"
      >
        ถ่ายหน้าเปิดประตู
      </button>
      ...
      <div
        v-if="alertMessage"
        class="p-4 mt-4 mb-4 text-white bg-red-500 rounded"
      >
        {{ alertMessage }}
      </div>
      <div
        v-if="alertMessageG"
        class="p-4 mt-4 mb-4 text-white bg-green-500 rounded"
      >
        {{ alertMessageG }}
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    async sendLineNotify(message, imageData) {
      try {
        const formData = new FormData();
        formData.append("message", message);
        formData.append(
          "api_key",
          "5eT2HLgd00FtZlYM4aEPMfok2CEd5MSTSvkU0IvCM0p"
        );
        if (imageData) {
        ...
        const response = await fetch(
          "https://vue.cpss.ac.th/line_notify_open.php",
          {
            method: "POST",
            body: formData,
    ...
    async compareCapturedImageWithFirestoreEmbeddings(faceEmbeddings) {
      ...
      if (users.length > 0) {
        ...
        users.forEach((user) => {
          user.face_embedding.forEach((embedding) => {
            if (
              ...
              faceEmbeddings[0].length === embedding.descriptor.length
            ) {
              ...
              console.error(
                `Length mismatch: ${faceEmbeddings[0]?.length} !== ${embedding.descriptor?.length}`
              );
            }
          });
        });
        ...
        const similarityPercentage = Math.max(
          0,
          (1 - lowestDistance / max_distance) * 100
        ).toFixed(2);

        if (this.bestMatch && similarityPercentage >= 70) {
          // this.showAlertG(`ยินดีต้อนรับ ${this.bestMatch.name}`);
          this.showAlertG(
            `ยินดีต้อนรับ ${this.bestMatch.name} (ความเหมือน: ${similarityPercentage}%)`
          );
        ...
        this.showAlert(
          "ไม่พบข้อมูลผู้ดูแลห้อง หรือเกิดความผิดพลาดในการดึงข้อมูล."
        );
    ...
    async processFaceRecognition(imageData) {
      await faceapi.nets.ssdMobilenetv1
        .loadFromUri("/models2/ssd_mobilenetv1_model-weights_manifest.json")
        .catch((err) =>
          console.error("Error loading ssdMobilenetv1 model:", err)
        );

      await faceapi.nets.faceLandmark68Net
        .loadFromUri("/models2/face_landmark_68_model-weights_manifest.json")
        .catch((err) =>
          console.error("Error loading faceLandmark68Net model:", err)
        );

      await faceapi.nets.faceRecognitionNet
        .loadFromUri("/models2/face_recognition_model-weights_manifest.json")
        .catch((err) =>
    ...
    async generateFaceEmbeddings(detections, img) {
      ...
      const isMatch = await this.compareCapturedImageWithFirestoreEmbeddings(
        this.faceEmbeddings
      );
      if (isMatch) {
        ...
        // ส่งภาพและข้อความไปยัง Line Notify
        const message = `เปิดประตูเรียบร้อยสำหรับ ${this.bestMatch.name} (${this.bestMatch.position}) ห้อง ${this.bestMatch.room_number}`;
        this.sendLineNotify(message, img.src);
